...
{
    private readonly Dictionary<Type, ComponentPool> _pools = new();
    ...
    public Entity Spawn<T>( in T component ) where T : Component
    {
        Entity entity = AllocateEntity();
    ...
    public void Despawn( Entity entity )
    {
        ...
        _versions[ entity.Id ]++;
        foreach ( ComponentPool pool in _pools.Values ) pool.Remove( entity.Id );
    ...
    public void Add<T>( Entity entity, in T component ) where T : Component
    {
    ...
    public void Remove<T>( Entity entity ) where T : Component
    {
        if ( !IsAlive( entity ) ) throw new InvalidOperationException( "Entity is not alive." );
    ...
    public ref T Get<T>( Entity entity ) where T : Component
    {
        if ( !IsAlive( entity ) ) throw new InvalidOperationException( "Entity is not alive." );
    ...
    public bool Has<T>( Entity entity ) where T : Component
    {
        ...
        if ( _pools.TryGetValue( typeof( T ), out ComponentPool? pool ) ) return pool.Has( entity.Id );
        return false;
    }
    ...
    {
        Type type = typeof( T );
        if ( !_pools.TryGetValue( type, out ComponentPool? pool ) )
        {
            pool = new ComponentPoolImpl<T>( _options.MaxEntities, _options.InitialCapacity );
            _pools[ type ] = pool;
        }

        return ( ComponentPoolImpl<T> )pool;
    }

    internal ComponentPoolImpl<T>? TryGetPool<T>() where T : Component
    {
        _pools.TryGetValue( typeof( T ), out ComponentPool? pool );
        return ( ComponentPoolImpl<T>? )pool;
    }
    ...
    {
        Cleanup();
        foreach ( ComponentPool pool in _pools.Values ) pool.Clear();
        _pools.Clear();
        _recycledIds.Clear();
        _systems.Clear();
        _globals.Clear();
