...
public static class LineDetection
{
    ...
    public static HashSet<Vector2D<int>> FindMatchingLines( int x, int y, int gridWidth, int gridHeight,
    ...
    private static void CheckLine( int x, int y, int dx, int dy, int gridWidth, int gridHeight,
        Func<int, int, int> getValue, int targetValue, int minLength, HashSet<Vector2D<int>> result )
    {
        List<Vector2D<int>> line = [ new( x, y ) ];
        ...
        for ( int i = 1;; i++ )
        {
            ...
            line.Add( new Vector2D<int>( nx, ny ) );
        }
        ...
        {
            foreach ( Vector2D<int> cell in line )
                result.Add( cell );
        }
    }
}
