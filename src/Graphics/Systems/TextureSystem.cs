...
public class TextureSystem( GL gl ) : SystemBase
{
    ...
    private Texture2DComponent CreateTextureFromPixels( byte[] pixels, int width, int height )
    {
    ...
    private static Slice[] CalculateNineSlices( Vector2D<float> position, Vector2D<float> size, int borderPixels,
        int textureSize )
    {
        ...
        [
            ...
            new Slice( new Vector2D<float>( x1, y2 ), new Vector2D<float>( x2, y3 ),
                new Vector2D<float>( borderUv, v2 ),
                new Vector2D<float>( u2, v3 ) ),
            new Slice( new Vector2D<float>( x2, y2 ), new Vector2D<float>( x3, y3 ), new Vector2D<float>( u2, v2 ),
                new Vector2D<float>( u3, v3 ) ),

            // Middle row
            new Slice( new Vector2D<float>( x0, y1 ), new Vector2D<float>( x1, y2 ),
                new Vector2D<float>( u0, borderUv ),
                new Vector2D<float>( borderUv, v2 ) ),
            new Slice( new Vector2D<float>( x1, y1 ), new Vector2D<float>( x2, y2 ),
                new Vector2D<float>( borderUv, borderUv ),
                new Vector2D<float>( u2, v2 ) ),
            new Slice( new Vector2D<float>( x2, y1 ), new Vector2D<float>( x3, y2 ),
            ...
            new Slice( new Vector2D<float>( x0, y0 ), new Vector2D<float>( x1, y1 ), new Vector2D<float>( u0, v0 ),
                new Vector2D<float>( borderUv, borderUv ) ),
            new Slice( new Vector2D<float>( x1, y0 ), new Vector2D<float>( x2, y1 ),
                new Vector2D<float>( borderUv, v0 ),
                new Vector2D<float>( u2, borderUv ) ),
