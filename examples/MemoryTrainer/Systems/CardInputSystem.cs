...
public class CardInputSystem : SystemBase
{
    ...
    protected override void OnUpdate( float deltaTime )
    {
        ...
        // Проверяем, ждем ли мы переворота карт обратно
        if ( gameState.IsWaitingForFlipBack )
        {
            gameState.DelayTimer += deltaTime;

            if ( gameState.DelayTimer >= 1f ) // 1 секунда задержки
            {
                ...
                if ( gameState.FirstRevealedCard.HasValue && World.IsAlive( gameState.FirstRevealedCard.Value ) )
                {
                    ...
                    if ( !card1.IsMatched )
                    {
                        ...
                        card1.IsRevealed = false; // Важно! Сбрасываем флаг
                    }
                }
                ...
                if ( gameState.SecondRevealedCard.HasValue && World.IsAlive( gameState.SecondRevealedCard.Value ) )
                {
                    ...
                    if ( !card2.IsMatched )
                    {
                        ...
                        card2.IsRevealed = false; // Важно! Сбрасываем флаг
                    }
                }
        ...
        // Обработка клика мыши (только если не ждем переворота)
        bool isMousePressed = input.Mouse.IsButtonPressed( MouseButton.Left );

        // Срабатываем только на переход из не нажатого в нажатое состояние
        if ( isMousePressed && !_wasMousePressed )
        {
            // Получаем позицию мыши
            System.Numerics.Vector2 mousePosNum = input.Mouse.Position;
            ...
            if ( cameraEntity.HasValue )
            {
                ...
                // Преобразуем координаты мыши в луч
                if ( UnprojectMouseToRay( mousePos, camera, out Vector3D<float> rayOrigin,
                        out Vector3D<float> rayDirection ) )
                {
                    // Проверяем пересечение с картами
        ...
        out Vector3D<float> rayOrigin, out Vector3D<float> rayDirection )
    {
        ...
        // Нормализуем координаты мыши в NDC (-1 до 1)
        float x = 2f * mousePos.X / width - 1f;
        float y = 1f - 2f * mousePos.Y / height; // Инвертируем Y

        // Используем кэшированные матрицы из камеры
        ...
        // Делим на W для получения реальных координат
    ...
    {
        // Плоскость карт (Y = 0, карты лежат горизонтально)
        float planeY = 0f;

        // Проверяем пересечение луча с плоскостью Y=0
        if ( MathF.Abs( rayDirection.Y ) < 0.0001f )
            return; // Луч параллелен плоскости

        float t = ( planeY - rayOrigin.Y ) / rayDirection.Y;
        if ( t < 0 )
            return; // Пересечение позади камеры
    ...
    private void RestartGame()
    {
        ...
        World.GetGlobal<MaterialSystem>();
