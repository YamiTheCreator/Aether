...
public class KleinBottleSystem : SystemBase
{
    protected override void OnUpdate( float deltaTime )
    {
        ...
        foreach ( Entity entity in World.Filter<KleinBottle>().With<Transform>() )
        {
            ...
            GenerateBottleMesh( entity, ref bottle, meshSystem, materialSystem );
        ...
        MaterialSystem materialSystem )
    {
        ...
        for ( int i = 0; i <= bottle.USegments; i++ )
        {
            ...
            for ( int j = 0; j <= bottle.VSegments; j++ )
            {
                ...
                // Цветовой градиент на основе Y координаты
                float t = ( y - minY ) / ( maxY - minY );
                Vector4D<float> color = new( 0.5f + t * 0.5f, 0.3f, 1f - t * 0.5f, 0.5f );
